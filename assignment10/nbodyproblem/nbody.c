/*
 * File: nbody.c
 * Purpose: A parallel N-body simulation using MPI.
 * Author: Sean Balbale
 * Date: 5/4/2026
 */
...
#define SOFTENING 0.05 // Plummer softening length epsilon (avoids r->0)
#define DT 0.01        // leapfrog timestep

// compute_forces()
// Computes softened gravitational forces on the local particles using all N positions.
...
// leapfrog_kick()
// Applies the leapfrog half-kick update to local velocities.
...
// leapfrog_drift()
// Advances the local position slice by one full timestep.
...
int main(int argc, char *argv[])
{
    ...
    // Ranks 0..(N%P - 1) each own one extra particle.
    ...
    if (rank == 0)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            // Particles uniformly distributed in [-1, 1]^3
